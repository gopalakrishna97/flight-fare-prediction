...
class DataValidation:
    ...
    def drop_missing_values_columns(self,df,report_key_name:str,threshold:float=0.3)->Optional[pd.DataFrame]:
        """
        this funtion will drop columns which contains missing values more than specified threshold
        
        df:Accepts pandas dataframe 
        threshold: Percentage criteria to drop the column
        ------------------------------------------------------------
        returns Pandas DataFrame if atleat single column available after droping missing columns
        """
        try:
    ...
    def is_required_columns_exits(self,base_df,current_df,report_key_name:str)->bool:
        try:
    ...
    def data_drift(self,base_df,current_df,report_key_name:str):
        try:
            ...
            for base_column in base_columns:
                ...
                # stats.ks_2samp return two values statistic ,pvalue
                statistic ,pvalue = stats.ks_2samp(base_data,current_data)
                # print("same_distribution,pvalue",statistic ,pvalue)
